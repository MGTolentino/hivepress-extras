...
function markRequiredExtras() {
    ...
    // Luego, verificar variable quantity required
...
// Añadir estilos para extras requeridos
...
window.fixRequiredExtras = function() {
    ...
    // Ahora marcar solo los que realmente son required
    
    // 1. Para los checkboxes - buscar primero los que son required
    $('input[name="_extras[]"][data-required="true"]').each(function() {
        var $input = $(this);
        var index = $input.val(); // Este es el índice del extra en el array
        
        
        // Por cada botón en las tarjetas, verificar si corresponde a este extra
        $('.hp-price-extra__reserve').each(function() {
            var $button = $(this);
            ...
            // Comprobar coincidencia exacta de nombres
    ...
    // 2. Para los inputs de variable quantity que son required
    $('.bv-extra-quantity[data-required="true"]').each(function() {
        var $input = $(this);
        var inputExtraName = $input.data('name') || '';
  ...
  $(document).ready(() => {
    ...
    initializePopups();
    preloadImages();
    // Eliminar updateReserveButtons();
    syncExtraButtons(); // Este ahora establecerá el texto inicial
    syncVariableQuantityInputs();
    
    // Añadir estilos y marcar extras required
...
$(document).on('change', '.bv-extra-quantity', function() {
   ...
   // Verificar extras required
   markRequiredExtras();
});


       // Observar cambios en los checkboxes del form
   ...
   // Agregar detección de carga de página completada
$(window).on('load', function() {
   ...
   $(document).on('hp-form.field.added', function(e, container) {
       ...
       // Añadido: verificar extras required
       setTimeout(function() {
           markRequiredExtras();
           window.fixRequiredExtras();
       }, 300);
	...
	// Manejar posibles actualizaciones AJAX
